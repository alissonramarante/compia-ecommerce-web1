...
export function ProductCard({ produto }: { produto: Produto }) {
  ...
  const esgotado = produto.formato === "fisico" && produto.estoque <= 0;
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border bg-background shadow-sm transition-shadow hover:shadow-md">
      <Link to={`/produtos/${String(produto.id)}`} className="block overflow-hidden">
        <img
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <h3 className="line-clamp-2 font-display text-base font-semibold leading-snug">
          <Link to={`/produtos/${produto.id}`} className="hover:underline">
            {produto.titulo}
          </Link>
        </h3>
        ...
        <div className="mt-auto space-y-3 pt-2">
          ...
          <div className="flex flex-col gap-2">
            <Button className="w-full whitespace-normal text-center leading-tight px-4 py-3"
            disabled={esgotado} onClick={() => addItem(produto)}>
